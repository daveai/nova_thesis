...
The $\gamma$ term is the key addition. If $\gamma$ is negative, bad news raises volatility more than good news of the same size, the leverage effect. This asymmetry is well documented in equity markets, though as noted earlier, it can run the other way in crypto.
...
For LSTM, all features are standardised to have zero mean and unit variance, with the mean and standard deviation computed on the training set only and then applied unchanged to the test set. This prevents information leakage and helps with training stability.
